...
const module: any = defineNuxtModule({
  ...
  async setup(_, nuxt) {
    ...
    nuxt.hook('prepare:types', (opts) => {
      const tsConfig: typeof opts.tsConfig & { vueCompilerOptions?: any } =
        opts.tsConfig;
      tsConfig.compilerOptions = tsConfig.compilerOptions || {};
      tsConfig.compilerOptions.paths = tsConfig.compilerOptions.paths || {};
      tsConfig.compilerOptions.paths['@huyooo-hub/tokens/config'] = [
        modulePath.resolve('./tokens.config.ts'),
      ];
    });
